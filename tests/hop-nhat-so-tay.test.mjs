...
import { hopNhatSoTay } from "../src/hop-nhat-so-tay.mjs";
...
test("7. Trang trắng tự sinh KHÔNG được làm sổ đã xoá sống lại", () => {
  // Máy A xoá sổ lúc 10:00. Máy B có bản cũ trong bộ nhớ, chỉ chạm vào nên
  // `getCurrentPage()` tự tạo một trang TRẮNG lúc 10:05. Sổ phải vẫn là đã xoá.
  const mayA = [
    {
      id: "so-1",
      ten: "Sổ cũ",
      taoLuc: "2026-09-09T08:00:00.000Z",
      suaLuc: "2026-09-09T10:00:00.000Z",
      daXoa: true,
      xoaLuc: "2026-09-09T10:00:00.000Z",
      trang: [{ id: "trang-1", taoLuc: "2026-09-09T08:00:00.000Z", suaLuc: "2026-09-09T10:00:00.000Z", daXoa: true, xoaLuc: "2026-09-09T10:00:00.000Z", scene: { items: [] } }]
    }
  ];
  const mayB = [
    {
      ...
      suaLuc: "2026-09-09T10:05:00.000Z",
      trang: [
        { id: "trang-1", taoLuc: "2026-09-09T08:00:00.000Z", suaLuc: "2026-09-09T09:00:00.000Z", scene: { items: [] } },
        { id: "trang-moi", taoLuc: "2026-09-09T10:05:00.000Z", suaLuc: "2026-09-09T10:05:00.000Z", scene: { items: [] } }
      ]
    }
  ];
  assert.equal(hopNhatSoTay(mayA, mayB)[0].daXoa, true);
  assert.equal(hopNhatSoTay(mayB, mayA)[0].daXoa, true);
});

test("8. Vẽ thật vào sổ đã xoá thì sổ vẫn sống lại", () => {
  const mayA = [
    {
      id: "so-1",
      ten: "Sổ cũ",
  ...
  const mayB = [
    {
      ...
      taoLuc: "2026-09-09T08:00:00.000Z",
      suaLuc: "2026-09-09T10:05:00.000Z",
      trang: [{ id: "trang-1", taoLuc: "2026-09-09T08:00:00.000Z", suaLuc: "2026-09-09T10:05:00.000Z", scene: { items: [{ type: "pen" }] } }]
    }
  ];
  assert.equal(hopNhatSoTay(mayA, mayB)[0].daXoa, false);
  assert.equal(hopNhatSoTay(mayB, mayA)[0].daXoa, false);
});
